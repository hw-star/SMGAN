...
class SMGANModel(BaseModel):
    def __init__(self, opt):
        super(SMGANModel, self).__init__(opt)
        if opt['dist']:
        ...
        if self.is_train:
            ...
            self.optimizer_D_grad = torch.optim.AdamW(self.netD_grad.parameters(), lr=train_opt['lr_D'], \
    ...
    def optimize_parameters(self, step):
        ...
        self.fake_H = self.netG(self.var_LQ)
        ...
        if step < self.l1_init:
          ...
          ####################################################################
          total_niter = self.opt['train']['niter']  # 假设你传了总训练步数进opt里
          warmup_iters = int(total_niter * 0.1)
          if step < self.l1_init:
            gan_scaling = 0.0
          elif step < self.l1_init + warmup_iters:
            gan_scaling = (step - self.l1_init) / warmup_iters
          else:
            gan_scaling = 1.0
          gan_weight = self.l_gan_w * gan_scaling
          grad_gan_weight = self.l_gan_grad_w * gan_scaling
          ######################################################################
          if step % self.D_update_ratio == 0 and step > self.D_init_iters:
            with torch.autograd.set_detect_anomaly(True):
              ...
              if self.opt['train']['gan_type'] == 'gan':
                ...
                l_g_gan = gan_weight * self.cri_gan(pred_g_fake, True)
              ...
              if self.opt['train']['gan_type'] == 'gan':
                ...
                l_g_gan_grad = grad_gan_weight * self.cri_gan(pred_g_fake_grad, True)
              elif self.opt['train']['gan_type'] == 'ragan':
